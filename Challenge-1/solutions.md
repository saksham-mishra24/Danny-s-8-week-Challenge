...
SELECT product_name, total_purchases FROM item_totals
WHERE total_purchases = 
(SELECT MAX(total_purchases) FROM item_totals)
...
```sql
WITH order_info AS
  (SELECT product_name,customer_id, count(product_name) AS order_count,
   ...
   GROUP BY customer_id, product_name)
SELECT customer_id,
GROUP_CONCAT(DISTINCT product_name ORDER BY product_name) AS product_name,order_count
FROM order_info
WHERE rank_num =1
GROUP BY customer_id;
```
...
```
 * Both RANK() and DENSE_RANK() are window functions that assign a ranking to each row within a specified partition of the result set, based on the values of one or more columns. However, there are some differences between them that may influence when to use one over the other.

 *  The main difference between RANK() and DENSE_RANK() is how they handle ties. When there are ties (i.e., multiple rows with the same value), RANK() assigns the same rank to each tied row and leaves gaps in the ranking sequence for the next unique value. For example, if the top three rows have the same value, their ranks will be 1, 1, and 1, and the next row will have a rank of 4. DENSE_RANK(), on the other hand, assigns the same rank to each tied row but does not leave any gaps in the ranking sequence. For example, if the top three rows have the same value, their ranks will be 1, 1, and 1, and the next row will have a rank of 2.

* So, when to use RANK() vs DENSE_RANK() depends on how you want to handle ties in your ranking. If you want to leave gaps in the ranking sequence for the next unique value, use RANK(). If you want to avoid gaps in the ranking sequence and assign the same rank to tied rows, use DENSE_RANK().

- For example, in the context of a sales report, you might use RANK() to rank sales representatives based on their total sales revenue, because you want to show the actual ranking of each representative even if they have the same total sales revenue. On the other hand, you might use DENSE_RANK() to rank customers based on the number of times they have purchased a particular product, because you want to show the correct ordering of customers even if multiple customers have purchased the product the same number of times.
```
***
